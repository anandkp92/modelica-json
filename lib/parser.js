...
/** Run the parser for the top-level `moFiles`,
  * and returns the RAW-JSON representations
  *
  ...
  */
function getRawJSON (moFiles, parseMode) {
  var rawJsonData = []
  if (parseMode === 'modelica') {
    for (var i = 0; i < moFiles.length; i++) {
      rawJsonData.push(getRawJsons(moFiles[i], parseMode))
    }
  } else {
    rawJsonData = getRawJsons(moFiles[0], parseMode)
  }
  return rawJsonData
...
/** Run the parser for each `moFile`,
  ...
  *@param parseMode Paring mode, 'cdl' or 'modelica'
  *@param outputFormat Output format, 'raw-json', 'json', 'html' or 'docx'
  *@return raw json data array. It could be one-element array (parseMode = 'modelica'),
  *        or multiple-elements array (parseMode = 'cdl')
  */
function getRawJsons (moFile, parseMode) {
...
/** Run the parser for the top-level `moFiles`,
  ...
  *@param moFiles Array of mo files to be parsed
  *@param parseMode Paring mode, 'cdl' or 'modelica'
  *@param outputFormat Output format, 'json', 'html' or 'docx'
  *@return parsed json data array
  */
function getJSON2 (rawJsonOutput, moFiles, parseMode, outputFormat) {
  var jsonData = []
  if (parseMode === 'modelica') {
    for (var i = 0; i < moFiles.length; i++) {
      jsonData.push(getJsons2(rawJsonOutput[i], moFiles[i], parseMode, outputFormat))
    }
  } else {
    jsonData = getJsons2(rawJsonOutput, moFiles[0], parseMode, outputFormat)
  }
  return jsonData
}
...
/** Run the parser for each `moFile`,
  ...
  *        or multiple-elements array (parseMode = 'cdl')
  */
function getJsons2 (rawJsonOutput, moFile, parseMode, outputFormat) {
  //logger.debug('Entered parser.getJSON for ' + moFile)
  const MODELICAPATH = process.env.MODELICAPATH

  //const jsonContent = mj.toJSON(moFile, parseMode, MODELICAPATH)
  const jsonContent = rawJsonOutput
  console.log("getjsons2, jsonContent = ", jsonContent)
  const topLevelWithin = jsonContent.within ? jsonContent.within[0] : null
...
function getJsons (moFile, parseMode, outputFormat) {
  ...
  const jsonContent = mj.toJSON(moFile, parseMode, MODELICAPATH)
  console.log("getjsons, jsonContent = ", jsonContent)
  const topLevelWithin = jsonContent.within ? jsonContent.within[0] : null
...
module.exports.getRawJSON = getRawJSON
